...
| D15 | Retrievals are immutable history snapshots. | Store title, URL, snippet, score, content hash, and optional R2 reference so past answers remain explainable. | A changed source does not alter a previously stored citation. | `retrieval_items` and lazy source loading are missing. |
...
| D24 | Third-party credentials stay in backend with least privilege. | iOS, command JSON, SSE, history, and logs must never contain provider secrets. | Secret scanning and integration authorization tests pass; revocation works. | Current provider credential policy is not formalized in the command contract. |
...
| D30 | External side effects use an Outbox/Worker and provider idempotency. | Database transactions cannot atomically include email, messaging, payment, or provider APIs. | Timeout becomes `unknown/retryable`; retry never duplicates a provider effect. | Current agent result flow is synchronous and lacks action attempts/outbox. |
...
| D33 | Rate-limit users, devices, SSE, commands, model fallback, and downloads. | Protects D1/AI cost and avoids reconnect storms. | Limits return stable error code and `retry_after`; normal usage remains unaffected. | No unified rate-limit policy exists. |
...
| D40 | Backend contracts are the single API schema source. | OpenAPI 3.1 plus embedded JSON Schema prevents iOS/backend field drift. | Schema lint, fixture validation, breaking-change CI, and generated examples pass. | No `backend/contracts/` directory exists. |
