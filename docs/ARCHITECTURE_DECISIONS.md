...
| D19 | Prove the architecture with three vertical actions: read-only, reversible, high-risk-confirmed. | This exercises query, safe mutation/undo, and confirmation without implementing every feature first. | Each action completes voice/draft → backend → event → history → iOS UI. | Backend vertical paths, strict local envelope generation, backend presentation, Undo, and token recovery exist; real signed-model and physical-device end-to-end proof remains. |
| D20 | First release gates are quantitative: ≥95% intent accuracy, 0 high-risk false executions, 0 duplicate side effects, and 1–2s local draft feedback. | Measurable gates prevent a demo from being mistaken for a reliable product. | Golden dataset, concurrency tests, latency metrics, and iPhone 13 thermal tests pass. | A 32-example locale-tagged opt-in real-model suite is checked in; the gated signed artifact and measured accuracy/latency/device evidence remain. |
...
| D27 | APNs is a wake-up/reminder channel, never the data source. | Push loss is safe because REST sync recovers state; payloads stay privacy-light. | Drop push delivery and confirm foreground/resume sync produces the same state. | Terminal command transitions send an opaque `command_id` wake hint, and iOS background handling performs authenticated REST reconciliation. Real APNs delivery and loss/recovery UAT remain. |
