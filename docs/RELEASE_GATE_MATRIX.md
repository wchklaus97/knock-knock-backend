...
| RG-07 | Human release approval | **Workflow implemented; protected environment pending** | Manual release and rollback workflows require exact identifiers and approval phrases, but GitHub `production` must still be configured with required reviewers and no administrator bypass. Record approval for remote migrations, provider/APNs secrets, feature flags, and model rollout. Follow `PRODUCTION_RELEASE_RUNBOOK.md`. | Product owner / release approver |
