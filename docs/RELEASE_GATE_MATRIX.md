...
| RG-05 | Physical iPhone and APNs gate | **Partial** | A signed Staging build installs for iPhone 13-class hardware, but the exact current revision still needs unlocked-device microphone/VAD/interruption/memory/thermal/crash execution. Real APNs delivery and simultaneous two-physical-device convergence remain separate gates. | iOS/Verification agent + unlocked iPhone 13 and second phone |
...
- `cargo fmt --all -- --check`
- `cargo test --all-targets` — 60 tests before the final confirmation-replay additions
