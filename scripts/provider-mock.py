#!/usr/bin/env python3
"""Deterministic local HTTPS-provider boundary substitute for lifecycle smoke.

This server is intentionally tiny and local-only. It never contacts a vendor,
persists no credentials, and implements only the response states needed by
...
class Handler(BaseHTTPRequestHandler):
    ...
    def log_message(self, format: str, *args: object) -> None:
        # Do not log Authorization, request bodies, or provider payloads.
    ...
    def message_status(self, key: str) -> None:
        ...
        self.send_json(
            200,
            {"provider_id": resource["provider_id"], "delivery_state": "delivered"},
        )
